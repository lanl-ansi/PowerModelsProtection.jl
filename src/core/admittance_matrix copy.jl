        ...
        if haskey(gen, "fault_model")
            if gen["fault_model"]["standard"] == IEEE2800
                ...
                elseif gen["fault_model"]["priority"] == "reactive"
                    ...
                    println("$(gen["gen_bus"]) $(ir1) $(ir2) $(gen["i_max"][1])")
                    i_pos = ir1
                    i_neg = ir2
                    m = max(1, (abs(i_pos) + abs(i_neg))/(gen["i_max"][1]))
                    i_pos = i_pos/m 
                    i_neg = i_neg/m
                    if abs(i_pos) - abs(gen["current_seq"][2]) < -.01*abs(gen["current_seq"][2]) || abs(i_pos) - abs(gen["current_seq"][2]) > .01*abs(gen["current_seq"][2])
                        delta = abs(i_pos) - abs(gen["current_seq"][2])
                        i_pos = (sign(i_pos)*delta*1im)*exp(1im*(angle(v_012[2])))
                    else
                        i_pos = 0.0
                    end
                    if abs(i_neg) - abs(gen["current_seq"][3]) < -.01*abs(gen["current_seq"][3]) || abs(i_neg) - abs(gen["current_seq"][3]) > .01*abs(gen["current_seq"][3])
                        delta = abs(i_neg) - abs(gen["current_seq"][3])
                        i_neg = (sign(i_neg)*delta*1im)*exp(1im*(angle(v_012[3])))
                    else
                        i_neg = 0.0
                    end
                    current_seq = gen["current_seq"] .+ [0; i_pos ; i_neg]
                    m = max(1, (abs(current_seq[2]) + abs(current_seq[3]))/(gen["i_max"][1]))
                    println(m)
                    i_pos = i_pos/m 
                    i_neg = i_neg/m
                    i_inj = _A * [0; i_pos ; i_neg] #.- gen["i_inj"]
                    # current_seq = gen["current_seq"] .+ [0; i_pos ; i_neg]
                    # gen["diff"] = gen["current_seq"] - [0; i_pos ; i_neg]
                    # if abs(current_seq[2]) + abs(current_seq[3]) > gen["i_max"][1]
                    #     i_pos = 0.0
                    #     i_neg = 0.0
                    #     i_inj = _A * [0; 0.0; 0.0] 
                    # end
                ...
                #         gen["delta_ir1"] = 0.0
                #         gen["delta_ir2"] = 0.0
                #         gen["i_inj"] = [0.0; 0.0; 0.0]
                #     # end
                #     delta_v1 = 0.0
                #     if abs(v_012[2]) < (1-gen["fault_model"]["ir1_dead_band"]) * bus["vbase"] * data["settings"]["voltage_scale_factor"] || abs(v_012[2]) > (1+gen["fault_model"]["ir1_dead_band"]) * bus["vbase"] * data["settings"]["voltage_scale_factor"] 
                #         delta_v1 = abs(v_012[2])/(bus["vbase"] * data["settings"]["voltage_scale_factor"]) - 1
                #         ir1 = gen["fault_model"]["delta_ir1"] * delta_v1 * gen["i_max"][1]
                #         abs(ir1) > gen["i_max"][1] ? ir1 = gen["i_max"][1] : ir1 = ir1
                #     else
                #         println("********")
                #         ir1 = 0.0
                #     end   
                ...
                #         println(abs.(v_012))
                #         ir2 = gen["fault_model"]["delta_ir2"] * delta_v2 * gen["i_max"][1]
                #         abs(ir2) > gen["i_max"][1]*.5 ? ir2 = gen["i_max"][1]*.5 : ir2 = ir2
                #     else
                #         ir2 = 0.0
                #     end
                #     ip_pos = 0.0
                ...
                #         gen["v1"] = delta_v1
                #         gen["v2"] = delta_v2
                #         if gen["fault_model"]["p_control"] == "D"
                #             m = max(1, (abs(ir1) + abs(ir2))/gen["i_max"][1])
                #             _ir1 = ir1/m
                #             _ir2 = ir2/m
                #             iq_pos = gen["ir1"] + (_ir1 - gen["ir1"])
                #             iq_neg = gen["ir2"] + (_ir2 - gen["ir2"])
                #             gen["ir1"] = iq_pos
                #             gen["ir2"] = iq_neg
                #         else
                ...
                #         iq_neg = 0.0
                #     end
                #     if abs(ip_pos + iq_pos*1im) > .001
                #         i_pos = (abs(ip_pos + iq_pos*1im) - gen["i+"]) * ((ip_pos + iq_pos*1im)/abs(ip_pos + iq_pos*1im)) * exp(1im*(angle(v_012[2]))) 
                #     else
                #         i_pos = 0.0
                #     end
                #     gen["i+"] = abs(ip_pos + iq_pos*1im)
                #     # println(abs(v_012[2]))
                #     # println(angle(v_012[2])*180/pi)
                #     if abs(iq_neg) > .001
                #         i_neg = (abs(iq_neg*1im) - gen["i-"]) * (iq_neg/abs(iq_neg)) * exp(1im*(angle(v_012[3])))
                #     else
                #         i_neg = 0.0
                #     end
                #     gen["i-"] = abs(iq_neg*1im) 
                #     println("$(gen["gen_bus"]) $(gen["i+"]) $(gen["i-"]) $(delta_v1) $(delta_v2) $(gen["i_max"][1])")
                #     i_inj = _A * [0; i_pos ; i_neg]
                #     gen["power+"] = conj(gen["i+"])*v_012[2]
                #     gen["power-"] = conj(gen["i-"])*v_012[3]
                #     gen["current"] = _A * [0; gen["i+"]; gen["i-"]]
                #     gen["i_inj"] = gen["i_inj"] .+ i_inj
                #     for (_j, j) in enumerate(gen["connections"]) 
                #         if j != 4
                #             delta_i[data["admittance_map"][(bus["bus_i"], j)], 1] += i_inj[j] 
                #         end
